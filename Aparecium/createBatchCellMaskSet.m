function [ output_args ] = createBatchCellMaskSet(groundTruthImageFolder, originalImageFolder, numBatchPerImage, outputFolder, batchSize)
...
for imageIndex = 1 : numel(groundTruthImageNames)
   ...
   se = strel('disk',2);
   cellularBinary2 = imdilate(cellularBinary, se);
   cellularGroundTruth = imfill(cellularBinary2 + groundTruthDouble >= 1, 'holes');
   numberOfWhitePixels = sum(sum(cellularGroundTruth));
   [whiteIndexRow, whiteIndexCol] = find(cellularGroundTruth == 1);
   ...
   [blackIndexRow, blackIndexCol] = find( ((~logical(groundTruthDouble)) & (~logical(cellularGroundTruth)) & (~outsideCellHaloBinary) == 1)); % regular background
   [blackIndexRowHalo, blackIndexColHalo] = find(outsideCellHaloBinary == 1);
   randomsHalo = randperm(numel(blackIndexRowHalo));
   randomsHalo = randomsHalo(1 : min([numberOfWhitePixels/2, numel(randomsHalo)]));
   randomsBG = randperm(numel(blackIndexRow));
   randomsBG = randomsBG(1 : (numberOfWhitePixels - min([numberOfWhitePixels/2, numel(randomsHalo)])));
   blackIndexRow = [ blackIndexRowHalo(randomsHalo); blackIndexRow(randomsBG)];
   blackIndexCol = [ blackIndexColHalo(randomsHalo); blackIndexCol(randomsBG)];
   for batchIndex = 1 : numel(blackIndexCol)
