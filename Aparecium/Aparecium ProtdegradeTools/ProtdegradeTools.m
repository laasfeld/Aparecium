function varargout = ProtdegradeTools(varargin)
% PROTDEGRADETOOLS M-file for ProtdegradeTools.fig
%      PROTDEGRADETOOLS, by itself, creates a new PROTDEGRADETOOLS or raises the existing
%      singleton*.
%
%      H = PROTDEGRADETOOLS returns the handle to a new PROTDEGRADETOOLS or the handle to
%      the existing singleton*.
%
%      PROTDEGRADETOOLS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PROTDEGRADETOOLS.M with the given input arguments.
%
%      PROTDEGRADETOOLS('Property','Value',...) creates a new PROTDEGRADETOOLS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ProtdegradeTools_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ProtdegradeTools_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ProtdegradeTools

% Last Modified by GUIDE v2.5 23-Jan-2019 00:39:05

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ProtdegradeTools_OpeningFcn, ...
                   'gui_OutputFcn',  @ProtdegradeTools_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ProtdegradeTools is made visible.
function ProtdegradeTools_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ProtdegradeTools (see VARARGIN)

% Choose default command line output for ProtdegradeTools
addApareciumToPath();
...
% UIWAIT makes ProtdegradeTools wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ProtdegradeTools_OutputFcn(hObject, eventdata, handles) 
