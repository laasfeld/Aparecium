function varargout = ExperimentConfigurationPanel(varargin)
% EXPERIMENTCONFIGURATIONPANEL M-file for ExperimentConfigurationPanel.fig
%      EXPERIMENTCONFIGURATIONPANEL, by itself, creates a new EXPERIMENTCONFIGURATIONPANEL or raises the existing
%      singleton*.
%
%      H = EXPERIMENTCONFIGURATIONPANEL returns the handle to a new EXPERIMENTCONFIGURATIONPANEL or the handle to
%      the existing singleton*.
%
%      EXPERIMENTCONFIGURATIONPANEL('CALLBACK',hObject,eventData,handles,...) calls the
%      local
%      function named CALLBACK in EXPERIMENTCONFIGURATIONPANEL.M with the given input
%      arguments.
%
%      EXPERIMENTCONFIGURATIONPANEL('Property','Value',...) creates a new EXPERIMENTCONFIGURATIONPANEL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ExperimentConfigurationPanel_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ExperimentConfigurationPanel_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ExperimentConfigurationPanel

% Last Modified by GUIDE v2.5 29-Jun-2017 11:18:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ExperimentConfigurationPanel_OpeningFcn, ...
                   'gui_OutputFcn',  @ExperimentConfigurationPanel_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ExperimentConfigurationPanel is made visible.
function ExperimentConfigurationPanel_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ExperimentConfigurationPanel (see VARARGIN)

% Choose default command line output for ExperimentConfigurationPanel
...
% UIWAIT makes ExperimentConfigurationPanel wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ExperimentConfigurationPanel_OutputFcn(hObject, eventdata, handles) 
...
structure.ExperimentConfigurationPanel = handles.ExperimentConfigurationPanel;
structure.ConfigurationFilesPanel = handles.ConfigurationFilesPanel;
structure.OpenPlateSimulator = handles.OpenPlateSimulator;
structure.MeasurementsPanel = handles.MeasurementsPanel;
structure.SaveConfigurationFile = handles.SaveConfigurationFile;
structure.LoadConfigurationFile = handles.LoadConfigurationFile;
structure.channels_Table = handles.channels_Table;
structure.saveMIDASChannelNames = handles.saveMIDASChannelNames;
structure.DimensionalityPanel = handles.DimensionalityPanel;
structure.ChooseDimensionalityText = handles.ChooseDimensionalityText;
structure.ChooseDimensionality = handles.ChooseDimensionality;
structure.treatments_Table = handles.treatments_Table;
try
set(structure.ExperimentConfigurationPanel, 'Parent', handles.parent);
catch
...
if allow
    ...
    handles = generateApareciumExperimentInput(handles, handles.pheraStarAsciiReader.experimentDataStructure);
    setChannels(handles);
    handles.midasTableController.setData(rawData);
    successBox('PHERAstar ASCII file successfully loaded', 'Success');
    guidata(hObject, handles);
else
end
% --------------------------------------------------------------------
...
handles.pheraStarAsciiReader = PheraSTARASCIIReader();
handles.pheraStarDatabaseReader = PheraStarDatabaseReader();
...
function menu_PheraSTARDatabase_Callback(hObject, eventdata, handles)
% hObject    handle to menu_PheraSTARDatabase (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if isequal(handles.fileName, [])
    allow = 1;
...
if allow
    ...
    handles = generateApareciumExperimentInput(handles, handles.pheraStarDatabaseReader.experimentDataStructure);
    setChannels(handles);
    handles.midasTableController.setData(rawData);
    handles = updateMidasHeaders(handles);
    successBox('Data from PHERAstar database successfully loaded', 'Success');
    guidata(hObject, handles);
else
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
menu_PheraSTARDatabase_Callback(hObject, eventdata, handles)
