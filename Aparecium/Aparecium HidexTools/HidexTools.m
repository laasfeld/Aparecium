function varargout = HidexTools(varargin)
% HidexTOOLS M-file for HidexTools.fig
%      HidexTOOLS, by itself, creates a new HidexTOOLS or raises the existing
%      singleton*.
%
%      H = HidexTOOLS returns the handle to a new HidexTOOLS or the handle to
%      the existing singleton*.
%
%      HidexTOOLS('CALLBACK',hObject,eventData,handles,...) calls the
%      local
%      function named CALLBACK in HidexTOOLS.M with the given input
%      arguments.
%
%      HidexTOOLS('Property','Value',...) creates a new HidexTOOLS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HidexTools_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HidexTools_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HidexTools

% Last Modified by GUIDE v2.5 20-Nov-2025 00:01:01

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HidexTools_OpeningFcn, ...
                   'gui_OutputFcn',  @HidexTools_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before HidexTools is made visible.
function HidexTools_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HidexTools (see VARARGIN)

% Choose default command line output for HidexTools
addApareciumToPath();
handles.output = hObject;
handles.HidexLumExcelReader = HidexLumExcelReader();
...
% UIWAIT makes HidexTools wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = HidexTools_OutputFcn(hObject, eventdata, handles) 
...
% hObject    handle to menu_Excel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if allow
    ...
    [fullFilePath, fileName] = fileChooser.userChooseHidexExcelFile();
    handles.HidexLumExcelReader.readFile(fullFilePath);
    handles.fileName = fileName;
    handles.MIDAS_table.setVisible('on');
    set(handles.MIDASInformationText,'String', ['PheraSTAR ASCII file ', fileName, ' loaded']);
    handles.experimentDataStructure = handles.HidexLumExcelReader.experimentDataStructure;
    rawData = sendDataToMidasTable(handles.HidexLumExcelReader.experimentDataStructure, handles.dimensionality);
    handles = generateApareciumExperimentInput(handles, handles.HidexLumExcelReader.experimentDataStructure);
    setChannels(handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
menu_Excel_Callback(hObject, eventdata, handles)
