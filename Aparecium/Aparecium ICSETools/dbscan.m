...
% modified for speed by Tõnis Laasfeld, 2017
...
for i=1:m
    if touched(i)==0;
       ...
       d2 = distSquared(ob(2:n),x(:,2:n)); % faster version
       ind = find(d2<=Eps^2)';
...
D=sqrt(sum((((ones(m,1)*i)-x).^2),2))'; % slightly faster
...
%                                                                 
% Output: 
% D2 - Euclidean distance squared (1,m)



[m,n]=size(x);
%D=sqrt(sum((((ones(m,1)*i)-x).^2)'));
D2=sum((((ones(m,1)*i)-x).^2),2); % slightly faster
