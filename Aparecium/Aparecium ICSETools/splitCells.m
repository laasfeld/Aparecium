...
while ~isempty(MergeIndex)
    ...
    GrownBinaryPatch = conv2(BinaryPatch,double(nhood),'same') > 0;
    Neighbors = bwPatch .*GrownBinaryPatch;
    % NeighborsNbr = setdiff(unique(Neighbors(:)),[0 CurrentObjectNbr]);
    
    % faster alternative
    Neighbors(Neighbors == 0 | Neighbors == CurrentObjectNbr) = [];
    NeighborsNbr = unique(Neighbors(:));
    ...
    OrigInterfaceIndex = cell(1, length(NeighborsNbr));
    for j = 1:length(NeighborsNbr)
        ...
        % Consider removing log for speed improvement, since the likelihood
        % ratio is only used in a way where the unlogarithmed values can be
        % compared witout any qualitative loss 
        ...
        % Optimization by Tõnis Laasfeld
        % if LikelihoodRation(j) is smaller or equal to 0 then the merge
        % will be rejected regardless of the value of eccentricity of the
        % merged object. Therefor it can be assigned a value of the eccentricity threshold since the
        % real value has no purpose
        if LikelihoodRatio(j) > 0
            ...
            %MergedEccentricity(j) = tmp(1).Eccentricity; older slower
            %code
            
            % new faster code
            
            MergedEccentricity(j) = calculateEcc(MergedObject);
        else
            MergedEccentricity(j) = MaxEccentricity;
        end
...
%         OrigInterfaceIndex{j} = cat(1,tmp.PixelIdxList); %#ok Ignore MLint
        ...
        % Optimized code by Tõnis Laasfeld
        % first version of optimization
        % alternativeTMP = regionprops(double(Interface), 'PixelIdxList');
        % NewInterfaceIndex = alternativeTMP.PixelIdxList;
        
        % second version of optimization
        
        NewInterfaceIndex = find(logical(Interface) == 1);
        
        rows = size(Interface, 1);
        %columns = size(Interface, 2);
        origRows = size(I_c, 1);
        %origColumns = size(I_c, 2);
        realCoordinate = zeros(numel(NewInterfaceIndex), 1);
        for pixel = 1 : numel(NewInterfaceIndex)
           column = ceil(NewInterfaceIndex(pixel)/rows);
           row = NewInterfaceIndex(pixel) - (column-1)*rows; 
           realCoordinate(pixel, 1) = (column + cmin - 2) * origRows + rmin + row - 1;
        end
        OrigInterfaceIndex{j} = cat(1, realCoordinate);
        % Optimized code end
        %% Original code and optimized code comparison
%         if ~isequal(tmp.PixelIdxList, realCoordinate)
%             warndlg('New method for finding tmp.PixelIdxList does not give the same result');
%             pause 
%         end
        %% Code comparison end
    end
