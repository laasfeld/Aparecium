...
function varargout = ICSETools(varargin)
% ICSETOOLS M-file for ICSETools.fig
%      ICSETOOLS, by itself, creates a new ICSETOOLS or raises the existing
%      singleton*.
%
%      H = ICSETOOLS returns the handle to a new ICSETOOLS or the handle to
%      the existing singleton*.
%
%      ICSETOOLS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ICSETOOLS.M with the given input
%      arguments.
%
%      ICSETOOLS('Property','Value',...) creates a new ICSETOOLS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ICSETools_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ICSETools_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @ICSETools_OpeningFcn, ...
                   'gui_OutputFcn',  @ICSETools_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ICSETools is made visible.
function ICSETools_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ICSETools (see VARARGIN)

% Choose default command line output for ICSETools
...
handles.imageAnalyzer = ICSEImageAnalyzer(); % Create a new object that analyzes the images
...
% UIWAIT makes ICSETools wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ICSETools_OutputFcn(hObject, eventdata, handles) 
...
switch get(hObject,'Value')
    ...
    case 1
        ...
        includedParameters = get(handles.listbox2, 'String');
        if isempty(cell2mat(cellfun(@(x) strfind(x,'image'), includedParameters, 'uniformOutput', 0))) % if image is already in the list then do not add it twice
            includedParameters = cat(1, includedParameters, 'image');
            set(handles.listbox2, 'String', includedParameters);
            handles = removeIncludedParamsFromPossibleParams(handles);
        end
end
...
possibleCharacteristics = BinaryImageCalculator.implementedParameters;
set(hObject, 'String', possibleCharacteristics);
...
function handles = removeIncludedParamsFromPossibleParams(handles)
    possibleParams = get(handles.listbox1, 'String');
    includedParams = get(handles.listbox2, 'String');
    for possibleIndex = numel(possibleParams) : - 1 : 1
        found = 0;
        for includedIndex = 1 : numel(includedParams)
            if strcmp(includedParams{includedIndex}, possibleParams{possibleIndex})
                possibleParams(possibleIndex) = [];
                found = 1;
                break;
            end
            if found
        ...
        set(handles.listbox1, 'String', possibleParams);
    end
...
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    ...
    standardCharacteristics = {'objectCount', 'diameter', 'image'};
    set(hObject, 'String', standardCharacteristics);
    set(hObject, 'max', 2)% this allows selection of multiple lines
    guidata(hObject, handles);
...
possibleParameters = get(handles.listbox1, 'String');
toInclude = possibleParameters(selected);
includedParameters = get(handles.listbox2, 'String');
includedParameters = cat(1, includedParameters, toInclude);
set(handles.listbox2, 'String', includedParameters);
handles = removeIncludedParamsFromPossibleParams(handles);
...
possibleParameters = get(handles.listbox2, 'String');
toInclude = possibleParameters(selected);
includedParameters = get(handles.listbox1, 'String');
includedParameters = cat(1, includedParameters, toInclude);
set(handles.listbox1, 'String', includedParameters);
if ~isempty(cell2mat(cellfun(@(x) strfind(x,'image'), possibleParameters(selected), 'uniformOutput', 0))) &&...
        get(handles.autoSaveBinaryImage, 'Value') % there will be a conflict
        answer = questdlg('Removing image from included parameters will disable the option to save binary images, click OK to disable saving of binary images, click cancel to keep image as a calculated parameter.', 'warning', 'OK', 'Cancel', 'Cancel');
        switch(answer)
            case 'OK'
                set(handles.autoSaveBinaryImage, 'Value', 0);
            case 'Cancel'
                for i = 1 : numel(selected)
                   if isequal(possibleParameters{i}, 'image')
                      selected(i) = [];
                      break;
                   end
                end
...
possibleParameters(selected) = [];
set(handles.listbox2, 'String', possibleParameters);
...
if ~isempty(eventdata) && sum(strcmp(fieldnames(eventdata), 'Indices')) % needed to make it work with both object (newer matlab versions) and struct (older ones) % check if the inceces exist, means that some cell was modified
...
switch handles.analysisMode % check if the user wants to analyze a completed measurement or observe an ongoing experiment
    case 'Completed' % the case of the already completed measurement
        ...
        handles.imageAnalyzer.setParametersToCalculate(get(handles.listbox2, 'String'));
