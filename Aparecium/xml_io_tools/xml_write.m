function DOMnode = xml_write(filename, tree, RootName, Pref, extras)
...
struct2DOMnode(DOMnode, root, tree, DPref.ItemName, DPref, extras);
...
%  =======================================================================
function [] = struct2DOMnode(xml, parent, s, TagName, Pref, extras)
...
elseif (isstruct(s))  % if struct than deal with each field separatly
  if ((nDim==2 && strcmpi(Pref.StructTable,'Html')) || (nItem>1 && Pref.StructItem))
    ...
    for i = 1 : numel(s)
        struct2DOMnode(xml, parent, s(i), TagName, Pref,[]); % recursive call
    end
  elseif (nItem>1) % use  <TagName>...<\TagName> <TagName>...<\TagName> notation
    ...
    for i = 1 : numel(extras)
        fieldNames = fieldnames(extras{i});
        fieldName = fieldNames{1};
        struct2DOMnode(xml, node, extras{i}.(fieldName), fieldName, Pref ,[]);
    end
    parent.appendChild(node);
  end
%% == node is a leaf node ==
